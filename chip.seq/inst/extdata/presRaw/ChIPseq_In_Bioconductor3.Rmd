...
## Transcription factor binding site or epigenetic marks. 
...
## Using other sources of gene sets.

We may wish to take advantage of other gene set annotation available to us from sources such as MSigDB. We could have generated our own sets of interesting genes which we want to test for association with our peaks. 

We can use our own custom gene sets in **goseq** by specifying the **gene2cat** argument to a data.frame containing genes and their corresponding geneset membership.
...
## Custom gene sets from MsigDB.

We can use the GSEAbase package again to extract a list of genesets from a MSigDB gmt file.
...
And we can convert then this to a data.frame suitable for goseq.
...
## Custom geneset testing

We can then test this new geneset in the same way as before. We just need to supply the new categories as the data.frame to the **gene2cat** parameter. 

```{r,eval=T,echo=T, eval=T, echo=T, warning=FALSE,tidy=FALSE}
library(goseq)
pwf=nullp(allGenesForGOseq,"mm10","knownGene",plot.fit=FALSE)
...
## Custom geneset results

Finally we can review the results as we have with other goseq results. Here we can see Myc targets in our top hits from the hallmarks genesets.
...
Often with ChIP-seq we may know the motif we are looking for or we can use a set of known motifs from a database such as a [JASPAR](http://jaspar.genereg.net).
...
---
## Accessing motifs from JASPAR using TFBStools
...
With this PWM we can use the **motifmatchr** package to scan our summits for the Myc motif and return the postions of the motifs.
...
We can export the Myc motif positions within peaks for use later in IGV or for metaplot visualisation.
...
Exercise on ChIP-seq data can be found [here](../../exercises/exercises/chipseq_part3_exercise.html)
